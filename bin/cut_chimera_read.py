...
reads = 0
for sam in files:
        if sam[-18:] != "_uniq_classify.sam" : continue
        print(sam)
        if reads == 0:
...
unjudge_file = open(inpath + "/" + input_main + "_unjudge.sam",'w')
normal_R1 = open(inpath + "/" + input_main + "_normal_001.fastq",'w')
chimera_R1 = open(inpath + "/" + input_main + "_chimera_001.fastq",'w')
unjudge_R1 = open(inpath + "/" + input_main + "_unjudge_001.fastq",'w')
...
while i < int(reads):
	...
	if normal > chimera and normal + chimera > 1:
		normal_file.write(mergesam + "\n")
		normal_R1.write("@" + data[0][0] + "_" + str(i%2) + "\n" + data[0][9] + "\n" + '+' + "\n" + data[0][10] + "\n")
	elif normal < chimera and normal + chimera > 1:
		chimera_file.write(mergesam + "\n")
		chimera_R1.write("@" + data[0][0] + "_" + str(i%2) + "\n" + data[0][9] + "\n" + '+' + "\n" + data[0][10] + "\n")
	else:
		unjudge_file.write(mergesam + "\n")
		unjudge_R1.write("@" + data[0][0] + "_" + str(i%2) + "\n" + data[0][9] + "\n" + '+' + "\n" + data[0][10] + "\n")
normal_file.close()
chimera_file.close()
unjudge_file.close()
normal_R1.close()
...
for line in before_cut:
	...
	for i in range(len(cutpoint)):
		if int(cutpoint[i][0:(len(cutpoint[i])-1)]) > 20 and int(cutpoint[i][0:(len(cutpoint[i])-1)]) < len(line[9]):
